//! bitbank periodical-brief — 参考実装（提案用）
//!
//! 複数銘柄の「商い状況ダイジェスト」（1銘柄3行）を定期実行向けに一発で出す。
//! LLM/人間の両方が読める圧縮テキストを出力し、生JSONを下流に流さない。
//!
//! bitbank public REST API を直接呼ぶ:
//!   GET https://public.bitbank.cc/{pair}/candlestick/{type}/{YYYY | YYYYMMDD}
//!   GET https://public.bitbank.cc/tickers
...
/// 同時 HTTP リクエスト数の上限（--concurrency / BITBANK_BRIEF_CONCURRENCY / 既定16）。
static SEM_SIZE: OnceLock<usize> = OnceLock::new();
...
fn fetch_semaphore() -> &'static Semaphore {
    ...
    S.get_or_init(|| {
        let n = *SEM_SIZE.get_or_init(|| {
            std::env::var("BITBANK_BRIEF_CONCURRENCY")
                .ok()
...
const USAGE: &str = "\
usage: periodical_brief [pairs...] [--top N | --all] [--concurrency N]
...
fn main() {
    ...
    while let Some(a) = args.next() {
        match a.as_str() {
            ...
            "--top" => match args.next().and_then(|v| v.parse().ok()) {
                Some(n) => top = Some(n),
                None => {
                    eprintln!("--top には正の整数が必要です\n{USAGE}");
                    std::process::exit(2);
                }
            },
            "--concurrency" => match args.next().and_then(|v| v.parse::<usize>().ok()) {
                Some(n) if n >= 1 => {
                    let _ = SEM_SIZE.set(n);
                }
                _ => {
                    eprintln!("--concurrency には 1 以上の整数が必要です\n{USAGE}");
                    std::process::exit(2);
                }
            },
    ...
    if all || top.is_some() {
        match ranked_pairs() {
            Ok(ranked) => {
                let n = top.unwrap_or(ranked.len()).min(ranked.len());
                pairs = ranked.into_iter().take(n).collect();
            }
            Err(e) => {
                eprintln!("failed to rank pairs: {e}");
                std::process::exit(1);
            }
        }
    } else if pairs.is_empty() {
        pairs = std::env::var("BITBANK_BRIEF_PAIRS")
